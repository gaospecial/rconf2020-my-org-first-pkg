...
`daily_users` 其中并没有 `date` 列，因此需要先利用 `date()` 新建一个列。

```{r}
daily_users <- tbl_daily_users(con) %>%
...
stop <- make_date(year = year, month = month + 1, day = 1L) - days(1)
...
iap_revenue_period <-
  daily_users %>%
  filter(date %in% {{ days_in_period }}) %>%
  summarize(iap_rev = sum(iap_revenue, na.rm = TRUE)) %>%
  pull(iap_rev)
